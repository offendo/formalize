...
def modify_dataset_hf(dataset, seed):
    ...
    train = dataset["train"].map(lambda batch: modify_batch(batch, seed, dataset['train']), batched=True, num_proc=8)
    validation = dataset["validation"].map(lambda batch: modify_batch(batch, seed, dataset['validation']), batched=True, num_proc=8)

    return DatasetDict(
        {
            "train": concatenate_datasets([dataset["train"], train]),
            "validation": concatenate_datasets([dataset["validation"], validation]),
        }
    )
...
def main(input_file, output_path, seed):
    ...
    if input_file.endswith(".json"):
        ...
        modified_data.save_to_disk(output_path)
